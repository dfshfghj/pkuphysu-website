...
from .models import Posts, Comments, Follow, Articles
...
def get_posts(current_user):
    ...
    if pid:
        ...
        if keyword:
            query = query.filter(Posts.text.ilike("%{}%".format(keyword)))
        if begin:
            begin = int(begin)
    ...
    followed_post_ids = {
            pf.post_id for pf in 
            Follow.query.filter(Follow.user_id == current_user.id, Follow.post_id.in_([p.id for p in posts])).all()
        }
...
def get_comments(current_user, id):
    ...
    if begin:
        begin = int(begin)
        filter_condition = Comments.cid < begin if sort == "desc" else Comments.cid > begin
...
def submit_comments(current_user):
    ...
    result = Comments.insert_comment(current_user.id, data["pid"], data["text"], data["quote"])
    if result:
...
def submit_article(current_user):
    data = request.get_json()
    result = Articles.insert_article(current_user.username, data["title"], data["content"], data["tag"])
    if result:
        return respond_success(message="上传成功")
    else:
        return respond_error(400, "")
