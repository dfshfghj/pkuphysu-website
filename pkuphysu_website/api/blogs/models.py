...
class Comments(db.Model):
    ...
    timestamp = db.Column(db.Integer, nullable=False)
    quote = db.Column(
        db.Integer, db.ForeignKey("comments.cid", ondelete="SET NULL"), nullable=True
    )

    user = relationship("User", foreign_keys=[user_id], backref="comments")

    quoted_comment = relationship(
        "Comments", remote_side=[cid], foreign_keys=[quote], uselist=False
    )
