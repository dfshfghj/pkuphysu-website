...
class Puzzle(db.Model):  # 存储当前谜语
    ...
    def put_in(cls, puzzle: dict):  # 加入
        ...
        mappings.append(
            {"line_type": "cover", "content": puzzle["cover"], "locked": "F"}
        )
        mappings.append(
            {"line_type": "questions", "content": puzzle["questions"], "locked": "F"}
        )
        mappings.append(
            {"line_type": "answers", "content": puzzle["answer"], "locked": "F"}
        )
        mappings.append(
            {
                "line_type": "explanation",
                "content": puzzle["explanation"],
                "locked": "F",
            }
        )

        # 添加 keywords
        for keyword, v in puzzle.get("keywords", {}).items():
            for ques_id, content in v.items():
                mappings.append(
                    {
                        "line_type": "keyword",
                        "keyword": keyword,
        ...
        # 添加 clues
        for keyword, v in puzzle.get("clue", {}).items():
            for ques_id, content in v.items():
                mappings.append(
                    {
                        "line_type": "clue",
