...
const fetchArticles = async (config = { mode: "page", count: 1 }) => {
    try {
        const res = await fetch(`${API_BASE}/api/blogs/articles?limit=10&${config.mode}=${config.count}`);
...
<template>
    <el-scrollbar style="height: 100vh;">
        <div :class="['menu-wrapper acrylic', { scrolled: isScrolled }]">
            <el-menu :class="{ scrolled: isScrolled }" mode="horizontal" :ellipsis="false" router
                        ...
                        <el-dropdown @command="handleCommand">
                            ...
                            <template #dropdown>
                                <el-dropdown-menu>
                                    ...
                                    <el-dropdown-item command="logout" divided style="color: #f56c6c"> 退出登录
                                    </el-dropdown-item>
                                </el-dropdown-menu>
                            </template>
                        </el-dropdown>
        ...
        <div class="articles-container">
            <div v-for="article in articles" :id="article.id" style="width: 100%;">
                <div class="article-card" @click="router.push(`/chat/articles/${article.id}`)">
                    <div class="card-body">
                        ...
                        <span style="font-size: 14px;"> {{ article.content.slice(0, 50) }} {{ article.content.length >
                            50 ?
                            "..." : "" }}</span>
